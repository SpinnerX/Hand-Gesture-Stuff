...
struct NumberGenerator{
	NumberGenerator(bool random_seed = true) : distribution(-1.0f, 1.0f){
		if (random_seed) gen = std::mt19937(rd());
		else gen = std::mt19937(0);
	}
	...
	static NumberGenerator& getInstance(){ return *s_instance; }

	static void initialize(){ s_instance = std::make_unique<NumberGenerator>(false); }
