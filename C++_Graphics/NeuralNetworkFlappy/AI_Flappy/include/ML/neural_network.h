...
inline float sigm(const float f){
...
struct Network{
	...
	Network(const uint64_t input_size_) : input_size(input_size_) { }

	Network(const std::vector<uint64_t>& layers_sizes) : input_size(layers_sizes[0]){

		for (uint64_t i(1); i < layers_sizes.size(); i++) addLayer(layers_sizes[i]);
	
	}

	void addLayer(const uint64_t neurons_count){
		if (!layers.empty()) layers.emplace_back(neurons_count, layers.back().getNeuronsCount());
		else layers.emplace_back(neurons_count, input_size);
	}
