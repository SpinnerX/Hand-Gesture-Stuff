...
struct Obstacle{
    ...
    template<typename T>
    float getLength(const sf::Vector2<T>& v)const
    {
...
struct Renderer{
	Renderer(const World& world_, sf::RenderTarget& target_) : world(world_), target(target_){}
	...
	void draw(const World& world){
		for (const Obstacle& obstacle : world.obstacles) draw(obstacle);
